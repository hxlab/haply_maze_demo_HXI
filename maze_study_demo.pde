...
void setup(){
   ...
   */
  haplyBoard          = new Board(this, "COM12", 0);
  ...
  // Change config here - Tianzheng
  
  int density=50; // 40 or 60
...
void draw(){
  ...
  if(renderingForce == false){
    ...
    if(gameStart){
      fill(0, 0, 0);
      textAlign(CENTER);
      text("Push the ball and square to the red circle (one at a time)", width/2, 60);
      textAlign(CENTER);
